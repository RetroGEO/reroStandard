...
// Helper functions, maybe move into UnityCGRero.cginc
...
//From http://wiki.unity3d.com/index.php?title=ReliefSpecular#ShaderLab_-_ReliefSpecular.shader
float ray_intersect_rm(sampler2D reliefmap, float2 dp, float2 ds, float bias)
{
	float lin = 10;
	float bin = 0.5*lin;
	float size = 1.0/lin; // current size of search window
	float depth = -bias; // current depth position
	int i;
	
	for(i = 0; i < lin-1; i++){ // search front to back for first point inside object 
	    
        half4 t;
        if(_heightChannel == 0)
		     t = tex2D(reliefmap,dp+ds*depth).r;
        else if(_heightChannel == 1)
             t = tex2D(reliefmap,dp+ds*depth).g;
		else if(_heightChannel == 2)
             t = tex2D(reliefmap,dp+ds*depth).b;
		else
             t = tex2D(reliefmap,dp+ds*depth).a;
		
		if(depth < t.w-bias)
			depth += size;
	}
	
	for(i = 0; i < bin; i++){ // recurse around first point (depth) for closest match
