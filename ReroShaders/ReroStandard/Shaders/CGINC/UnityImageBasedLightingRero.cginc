                                    ...
                                    uint sampleCount = 2048)
{
    ...
    float3 lightCol = ShadeSH9(float4(0,0,0,1)) + _LightColor0;
    float wBrightness = saturate((lightCol.r + lightCol.g + lightCol.b)/3);
...
// For now disabled
	...
	if(_UseProbe)
	{
        ...
        {
		    rgbm = texCUBElod(_Cube, half4(R, mip));
            skycolor = DecodeHDR(rgbm, _Cube_HDR);
        }
	}
	else
	{
	...
	skycolor *= saturate(wBrightness * 100);
	return skycolor;
}
